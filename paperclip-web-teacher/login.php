<?php
// Start the session to manage user login state
session_start();

// Include the database connection file
require_once 'db_connect.php';

// Initialize an error message variable
$error_message = '';

// Check if the login form has been submitted
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get the submitted identification and password
    $identification = trim($_POST['identification'] ?? '');
    $password = trim($_POST['password'] ?? '');

    // Validate inputs
    if (empty($identification) || empty($password)) {
        $error_message = "Please enter both identification and password.";
    } else {
        // Prepare a SELECT statement to fetch user data based on identification
        $sql = "SELECT id, identification, password FROM users WHERE identification = ?";

        if ($stmt = $mysqli->prepare($sql)) {
            // Bind parameters to the prepared statement (s = string)
            $stmt->bind_param("s", $param_identification);

            // Set parameters
            $param_identification = $identification;

            // Attempt to execute the prepared statement
            if ($stmt->execute()) {
                // Store result
                $stmt->store_result();

                // Check if identification exists, then verify password
                if ($stmt->num_rows == 1) {
                    // Bind result variables
                    $stmt->bind_result($id, $identification_db, $hashed_password); // Use identification_db to avoid conflict
                    if ($stmt->fetch()) {
                        // Verify the password
                        if (password_verify($password, $hashed_password)) {
                            // Password is correct, so start a new session
                            $_SESSION['loggedin'] = true;
                            $_SESSION['id'] = $id; // Store user ID
                            $_SESSION['identification'] = $identification_db; // Store identification (e.g., 'teacher1')

                            // Construct the dynamic student table name and store it in the session
                            // IMPORTANT: Ensure your teacher identifications match the table naming convention
                            $_SESSION['student_table_name'] = 'students_' . strtolower($identification_db);

                            // Redirect to dashboard page
                            header("Location: dashboard.php");
                            exit;
                        } else {
                            // Password is not valid
                            $error_message = "Invalid identification or password.";
                        }
                    }
                } else {
                    // Identification doesn't exist
                    $error_message = "Invalid identification or password.";
                }
            } else {
                $error_message = "Oops! Something went wrong. Please try again later.";
            }

            // Close statement
            $stmt->close();
        }
    }

    // Close connection
    $mysqli->close();
}
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Paperclip - Teacher's Edition Login</title>
    <link rel="stylesheet" href="css/login.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link
        href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&display=swap"
        rel="stylesheet">
</head>

<body>
    <div class="container">
        <div class="logo">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 94 94" fill="#7D59FF">
                <mask id="mask0_15_33" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="94"
                    height="94">
                    <rect width="94" height="94" fill="#7D59FF"/>
                </mask>
                <g mask="url(#mask0_15_33)">
                    <path
                        d="M46.0208 86.1667C39.2319 86.1667 33.4548 83.784 28.6895 79.0188C23.9243 74.2535 21.5416 68.4764 21.5416 61.6875V32.5083L5.48328 16.45L10.9666 10.9667L83.0333 83.0334L77.55 88.5167L65.5062 76.4729C63.2215 79.4104 60.3982 81.7604 57.0364 83.5229C53.6746 85.2854 50.0027 86.1667 46.0208 86.1667ZM29.3749 40.3417V61.6875C29.3097 66.3222 30.909 70.2552 34.1729 73.4865C37.4368 76.7177 41.3861 78.3334 46.0208 78.3334C48.9583 78.3334 51.6184 77.6479 54.001 76.2771C56.3836 74.9063 58.3256 73.0785 59.827 70.7938L54.9312 65.8979C53.952 67.334 52.6954 68.4601 51.1614 69.2761C49.6274 70.092 47.9138 70.5 46.0208 70.5C43.018 70.5 40.4722 69.4556 38.3833 67.3667C36.2944 65.2778 35.2499 62.732 35.2499 59.7292V46.2167L29.3749 40.3417ZM43.0833 54.05V59.7292C43.0833 60.5778 43.3607 61.2795 43.9156 61.8344C44.4704 62.3893 45.1722 62.6667 46.0208 62.6667C46.8041 62.6667 47.4732 62.4056 48.0281 61.8834C48.5829 61.3611 48.893 60.7083 48.9583 59.925L43.0833 54.05ZM62.6666 51.5042V23.5H70.5V59.3375L62.6666 51.5042ZM48.9583 37.7958V25.4583C48.893 22.7167 47.9302 20.3993 46.0697 18.5063C44.2093 16.6132 41.9083 15.6667 39.1666 15.6667C37.4694 15.6667 35.9354 16.0583 34.5645 16.8417C33.1937 17.625 32.0513 18.6695 31.1374 19.975L25.5562 14.3938C27.1881 12.3701 29.1791 10.7708 31.5291 9.59584C33.8791 8.42084 36.4249 7.83334 39.1666 7.83334C44.0624 7.83334 48.2239 9.54689 51.651 12.974C55.0781 16.4011 56.7916 20.5625 56.7916 25.4583V45.6292L48.9583 37.7958ZM43.0833 23.5V31.9208L35.2499 24.0875V23.5H43.0833Z"
                        fill="#7D59FF" />
                </g>
            </svg>
        </div>
        <h1 class="app-name">paperclip</h1>
        <div class="app-subtitle">anti-cheating app | Teacher's Edition</div>

        <div class="login-instruction">
            Please log-in using the given logon<br>
            credentials by your supervisor/s.
        </div>

        <div class="login-card">
            <h2 class="login-title">Teacher Login</h2>
            <p class="login-description">Access the Student Data Table to monitor student analysis.</p>

            <form action="login.php" method="POST">
                <?php if ($error_message): ?>
                    <p style="color: red; text-align: center; margin-bottom: 10px;"><?php echo htmlspecialchars($error_message); ?></p>
                <?php endif; ?>
                <div class="form-group">
                    <label for="identification">Identification Code</label>
                    <input type="text" id="identification" name="identification" required>
                </div>

                <div class="form-group">
                    <label for="password">Password</label>
                    <input type="password" id="password" name="password" required>
                </div>

                <button type="submit" class="login-button">Log In</button>
            </form>
        </div>

        <div class="footer">
            The Paperclip App | Hosted by TSHS @ STEM
        </div>
    </div>
</body>

</html>
